...
import { uploadImageFormSchema, UploadSingleImageFormValues } from "@web/app/upload/_hooks/useImageUploadsForm";
import { usePromise } from "@web/hooks/usePromise";
import { ImageUpload } from "@web/components/modals/actions";
import { handleUpdateMedia } from "@web/app/account/media/actions";
      ...
      <Form {...form}>
         <form onSubmit={form.handleSubmit(onSubmit)}
            ...
            <FormField
               ...
               name={`tags`}
               render={({ field }) => (
                  <FormItem className={`!mt-0 w-fit`}>
                     <FormLabel>
                        <div className={`flex items-center gap-2`}>
                           <span className={`font-semibold text-neutral-300 text-base`}>Tags</span>
                           <TooltipProvider>
                              <Tooltip>
                                 <TooltipTrigger className={`cursor-auto`}>
                                    <CircleHelp className={`text-neutral-500`} size={14} />
                                 </TooltipTrigger>
                           ...
                           <span className={`text-xs font-light text-neutral-400`}>Enter tags in English.</span>
                        </div>

                     </FormLabel>
                     <FormControl className={`!mt-2 relative w-fit`}>
                        <div
                  ...
                  <FormItem className={`!mt-8`}>
                     <FormLabel>
                        <div className={`flex items-center justify-between w-full`}>
                           <div className={`flex items-center gap-2`}>
                              <span className={`font-semibold text-neutral-300 text-base`}>Description</span>
                              <TooltipProvider>
                                 <Tooltip>
                                    <TooltipTrigger className={`cursor-auto`}>
                                       <CircleHelp className={`text-neutral-500`} size={14} />
            ...
            <FormField
               ...
               render={({ field }) => (
                  <FormItem className={`!mt-4 flex items-center gap-3`}>
                     ...
                     <FormLabel className={`text-neutral-500 !mt-0 font-normal text-sm`}>
