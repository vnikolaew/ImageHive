...
const FUNCTION_ID = `image-commented`;

const EVENT_NAME: keyof Events = `image/image.commented`;

const POPULARITY_SCORE_KEY = `popularity_score`;

export const handleImageCommented = inngest.createFunction(
         ...
         image = await xprisma.image.update(
            {
               ...
               data: {
                  metadata: {
                     // @ts-ignore
                     ...(image.metadata ?? {}),
                     [POPULARITY_SCORE_KEY]: (image.metadata?.[POPULARITY_SCORE_KEY] ?? 0) + 1.2,
                  },
               },
